...
class Gate1(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.hidden_size = hidden_size
        self.wra = nn.Linear(hidden_size, hidden_size)
        self.wru = nn.Linear(hidden_size, hidden_size)


    def forward(self, a, prev_title):
        r_gate = torch.sigmoid(self.wra(a) + self.wru(prev_title))
        result =  torch.mul(r_gate, a) + torch.mul(torch.add(-r_gate, 1), prev_title)
        return result
...
class DecoderRNNFB(BaseRNN):
    ...
    def __init__(self, vocab_size, embedding, max_len, embed_size,
        ...
        self.attention_prev_title = Attention(self.hidden_size)
        #self.fc = nn.Linear(self.hidden_size*2, self.hidden_size)
        self.fc = Gate(self.hidden_size)
        self.fc1 = Gate1(self.hidden_size)
        self.out1 = nn.Linear(self.hidden_size, self.output_size)
        self.out2 = nn.Linear(self.hidden_size, self.output_size)

    def forward_step(self, input_var, pt_encoder_states, pg_encoder_states, hidden, encoder_outputs, function):
        ...
        if pg_encoder_states is None:
            output_states_a = output_states_attn1
        else:
            output_states_attn2, attn2 = self.attention_hidden(output_states, pg_encoder_states)
            output_states_a = self.fc(output_states_attn1, output_states_attn2)
        if pt_encoder_states is None:
            output_states_attn = output_states_a
        else:
            output_states_attn3, attn3 = self.attention_prev_title(output_states, pt_encoder_states)
            output_states_attn = self.fc1(output_states_a, output_states_attn3)
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None,pt_encoder_states=None,
        ...
        decoder_hidden = decoder_hidden.view(1,decoder_hidden.size()[-2],768)
        
        use_teacher_forcing = True if teacher_forcing_ratio == 1 else False
        # use_teacher_forcing means using the ground truth from the last time step
        # as the input of the current time step
        # to avoid the negative influence of bad output from the network during training
        if use_teacher_forcing:
            decoder_input = inputs[:, :-1]
            
            decoder_outputs, decoder_output_states, decoder_hidden, attn = \
                self.forward_step(decoder_input, pt_encoder_states, pg_encoder_states,
            ...
            for di in range(max_length):
                decoder_output, decoder_output_state, decoder_hidden, step_attn = \
                    self.forward_step(decoder_input, pt_encoder_states, pg_encoder_states, decoder_hidden,
                                      encoder_outputs, function=function)
                # # not allow decoder to output UNK
                decoder_output[:, :, 3] = -float('inf')
                #print("SIZE: ", decoder_output.size(), decoder_output_state.size(), decoder_hidden.size())
    ...
    def _init_state(self, encoder_hidden):
        if encoder_hidden is None:
            return None
        # Concatenate the hidden states for two directions if the rnn is bidirectional
