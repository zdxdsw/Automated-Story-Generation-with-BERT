...
class Predictor(object):
    ...
    def predict(self, src_seq, num_exams):
        ...
        prev_generated_seq = input_variable
        outputs = []
    ...
    def predict_batch(self, source, prev_title, input_lengths, num_exams):
        torch.set_grad_enabled(False)
        output_seq = []
        ...
        prev_generated_seq = input_variables
        for k in range(num_exams):
            _, _, other = \
                self.model(input_variables, prev_title, prev_generated_seq, input_lengths)
            length = other['length']
    ...
    def predict_seq_title(self, title, sec_seq, num_exams):
        ...
        Returns:
            tgt_seq (list): list of tokens in target language as predicted by the pre-trained model
        """
        torch.set_grad_enabled(False)
        text = []
        for tok in title:
